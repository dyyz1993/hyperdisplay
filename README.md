...
- **零点击恢复**：Android 保存主机、设备身份、布局和画中画参数；同一设备以固定 EDID
  `(vendor, product, serial)` 恢复相同屏幕槽位。临时切后台保留 5 秒，5 秒内返回不销屏；滑掉
  任务/明确退出立即移除该设备的虚拟屏（进程被系统杀掉则 Host 约 6 秒兜底回收）。USB/Wi-Fi
  换路由则静默重连并复用屏幕身份。无客户端时没有虚拟屏、采集或编码。
- **多块虚拟显示器 + 平板分屏**：平板「布局」一次保存单屏、左右/上下分屏、侧栏或画中画，
  下次 HELLO 由 Host 统一恢复完整屏幕组；Android 不再直接逐块 CREATE/DESTROY，避免拓扑 churn。
- **原生像素 + Retina 尝试**：平板请求的完整像素用于 ScreenCaptureKit 采集、硬件编码和
  Android 渲染；系统接受时 macOS 使用一半大小的逻辑画布，例如 2800×1840 像素对应
  1400×920 点。部分竖向分屏尺寸在当前 macOS 上会忽略 2x 请求，此时 Host 保留同一块
  原生像素 1x 显示器并诚实标示，不会通过循环销毁重建来强求 HiDPI。
...
| M4 | 平板分屏多路显示（区域原生像素 + Retina 2x） | ✅ 完成（真机 2 屏并发 51fps；2x 语义已验证） |
