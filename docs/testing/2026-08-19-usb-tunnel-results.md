# USB 有线（adb 隧道）实测记录（2026-08-19）

> **2026-08-25 恢复生效。** 8-24 曾按「实时负载一律 UDP」删除本隧道；当日用户定稿
> AGENTS §1 例外条款后恢复：**Type-C 有线 adb reverse 隧道允许 TCP**（有线无损，
> RTT 0.16ms 优于直连 UDP），实现为 host 内置 `UsbTunnelController`（取代本文的
> python 桥接），客户端 `Transport.TUNNEL`。视频/输入在隧道上走 TCP 帧互转；
> Wi-Fi 与 USB 网络共享路径仍一律 UDP。没有可用路径时客户端显式提示，不静默降级。
